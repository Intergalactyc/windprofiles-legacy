...
FIGTAB = "C:\\Users\\22wal\\Code\\GLWind\\windprofiles-legacy\\results\\fig_tabular"
...
HEIGHTS = [
    ...
    32.0,
    106.0,
]  # Heights that we are concerned with for plotting, in meters. 80m is left out here.
...
HEIGHTS_GAPS = [
    ...
    80.0,
    106.0,
]  # Every height (HEIGHTS but with 80 as well), for the data gaps visualization

ZVALS = np.linspace(0.0, 157.5, 400)  # Linspace for plotting heights
UVALS = np.linspace(0, 17, 360)  # Linspace for plotting wind speed distributions
TERRAINS = ["open", "complex"]
DISPLACEMENTS = {"open": 0.0, "complex": 0.0}  # 20 * 2 / 3}
...
DISTS_BY_TERRAIN = (
    True  # Show separate terrain classes in speed_distributions and pti_distributions?
)

CORRELATION_VARIABLES = (
    [f"ws_{b}" for b in BOOMS_GAPS]
    + [f"t_{b}" for b in BOOMS_GAPS]
    + [f"p_{b}" for b in BOOMS_GAPS]
    + [f"rh_{b}" for b in BOOMS_GAPS]
    + [f"pti_{b}" for b in BOOMS_GAPS]
    + [f"drms_{b}" for b in BOOMS_GAPS]
    + ["vpt_2", "vpt_6", "vpt_lapse", "alpha", "Ri_bulk"]
)
...
def bar_stability(df, cid, summary, size, saveto, poster, details):
    ...
    if summary["stability_classes"] == 4:
        ax.bar(
            ...
            height=[
                stability_percents["unstable"],
                stability_percents["neutral"],
                stability_percents["stable"],
                stability_percents["strongly stable"],
            ],
            color=[
                COLORS["unstable"],
                COLORS["neutral"],
                COLORS["stable"],
                COLORS["strongly stable"],
            ],
        )
        for i, sc in enumerate(["unstable", "neutral", "stable", "strongly stable"]):
            ax.text(
                i,
                stability_percents[sc] + 1.25,
                f'{"N=" if i == 0 else ""}{len(df[df["stability"] == sc])}',
                ha="center",
                va="center",
            )
        ...
        raise Exception(
            f"Cannot handle {summary['stability_classes']} stability classes in plot 'bar_stability'"
        )
    if poster:
        ax.set_title(
            "Frequency of Wind Data Sorted by \nBulk Richardson Number Thermal Stability Classification"
        )
    ax.set_ylim(0, max(stability_percents) + 3)
    ax.set_ylabel("Proportion of Data (%)")
    ax.grid(axis="y", linestyle="-", alpha=0.75)
    plt.savefig(
        saveto, bbox_inches="tight", edgecolor=fig.get_edgecolor(), transparent=poster
    )
    return
...
def annual_profiles(df, cid, summary, size, saveto, poster, details):
    ...
    fig, axs = plt.subplots(
        1, 2, figsize=size, sharey=True, linewidth=5 * poster * neatline, edgecolor="k"
    )
    if summary["stability_classes"] == 4:
        ...
        raise Exception(
            f"Cannot handle {summary['stability_classes']} stability classes in plot 'annual_profiles'"
        )
    ...
    for i, tc in enumerate(TERRAINS):
        ...
        for sc in stabilities:
            ...
            def plot_fit(
                booms, heights, linestyle, scatter=False
            ):  # linestyle can be 'solid', 'dashed', 'dotted', 'dashdot', or a parameterization tuple
                nonlocal zorder
                ...
                ax.plot(
                    mult * ZVALS**wsc,
                    ZVALS,
                    color=change_luminosity(COLORS[sc], 0.85),
                    zorder=zorder,
                    linestyle=linestyle,
                )
                zorder += 2
                if scatter:  # enable this for the full line
                    ax.scatter(
                        means,
                        heights,
                        label=r"{sc}: $u(z)={a:.2f}z^{{{b:.3f}}}$".format(
                            sc=short, a=mult, b=wsc
                        ),
                        color=COLORS[sc],
                        zorder=6,
                        s=75,
            ...
            plot_fit(
                booms=BOOMS, heights=HEIGHTS, linestyle="solid", scatter=True
            )  # full (5-height) fit line
        ...
        out_table.to_csv(
            os.path.join(FIGTAB, "annual_profiles_fig8.csv"),
            index_label="height",
            float_format="%.6f",
        )
        ...
        if poster:
            tc_title = (
                r"Open Terrain (${openL}-{openR}\degree$ at {b})".format(
                    openL=int(135 - summary["terrain_window_width_degrees"] / 2),
                    openR=int(135 + summary["terrain_window_width_degrees"] / 2),
                    b=summary["terrain_wind_boom"],
                )
                if tc == "open"
                else r"Complex Terrain (${complexL}-{complexR}\degree$ at {b})".format(
                    complexL=int(315 - summary["terrain_window_width_degrees"] / 2),
                    complexR=int(315 + summary["terrain_window_width_degrees"] / 2),
                    b=summary["terrain_wind_boom"],
                )
            )
...
def wse_histograms(df, cid, summary, size, saveto, poster, details):
    ...
    if summary["stability_classes"] == 4:
        ...
        raise Exception(
            f"Cannot handle {summary['stability_classes']} stability classes in plot 'wse_histograms'"
        )
    fig, axs = plt.subplots(
        nrows,
        ncols,
    ...
    for i, ax in enumerate(fig.axes):
        ...
        for j, tc in enumerate(TERRAINS):
            ...
            if details:
                ...
                print(
                    f'\tMed Ri_b: {dfs.loc[dfs["terrain"] == tc, "Ri_bulk"].median():.2f}'
                )
        if i == ncols - 1:
        ...
        ax.vlines(
            x=[dfs.loc[dfs["terrain"] == tc, "alpha"].median() for tc in TERRAINS],
            ymin=0,
            ymax=ax.get_ylim()[1],
            colors=[change_luminosity(COLORS[tc], 1.5) for tc in TERRAINS],
            alpha=0.75,
            linestyle="dashed",
            linewidth=4,
        )
...
def veer_profiles(df, cid, summary, size, saveto, poster, details):
    ...
    for i, tc in enumerate(TERRAINS):
        ax = axs[i]
        dft = df[df["terrain"] == tc]
        for j, sc in enumerate(stabilities):
            dfs = dft[dft["stability"] == sc]
            means = [polar.unit_average_direction(dfs[f"wd_{b}"]) for b in BOOMS]
            stds = [dfs[f"wd_{b}"].std() for b in BOOMS]
            ...
            ax.scatter(
                ...
                marker=MARKERS[sc],
                facecolors="none",
                edgecolors=COLORS[sc],
                linewidths=1.5,
            )
            ax.errorbar(
                means,
                [h - 1.5 + j for h in HEIGHTS],
                xerr=stds,
                color=COLORS[sc],
                fmt=".",
                linestyle=None,
            )
            wdlabel = f"wd_{tc}_{sc.replace(' ','-')}"
            out_table[wdlabel] = means
            out_table[f"rms_wd_{wdlabel}"] = stds

        out_table.to_csv(
            os.path.join(FIGTAB, "veer_profiles_fig11.csv"),
            index_label="height",
            float_format="%.6f",
        )
...
def tod_wse(df, cid, summary, size, saveto, poster, details):
    ...
    COLORS = COLORS_POSTER if poster else COLORS_FORMAL
    fig, axs = plt.subplots(
        nrows=summary["stability_classes"],
        ncols=1,
        figsize=size,
        sharex=True,
        linewidth=5 * poster * neatline,
    ...
    # else:
    #     raise Exception(
    #         f"Cannot handle {summary['stability_classes']} stability classes in plot 'tod_wse'"
    #     )
    ...
    for i, ssn in enumerate(["fall", "winter", "spring", "summer"]):
        ...
        for j, tc in enumerate(TERRAINS):
            ...
            hourly_wse = [
                dft[dft["time"].dt.hour == hr]["alpha"].reset_index(drop=True)
                for hr in range(24)
            ]
            hourly_wse.append(
                dft[dft["time"].dt.hour == 0]["alpha"].reset_index(drop=True)
            )  # have 0 at both the start and end
            ...
            ax.errorbar(
                x=np.array(range(25)) + OFFSET * j,
                y=med_wse,
                yerr=std_wse,
                color=COLORS[tc],
                fmt=MARKERS[tc],
                markersize=12,
                label=r"$\alpha$ ({terr})".format(terr=tc.title()),
            )
            fitsine, params = stats.fit_sine(
                range(24), med_wse[:24], std_wse[:24], fix_period=True
            )
            if details:
                print(
                    f"\t{ssn} alpha = {params[0]:.4f} * sin({params[1]:.4f} * t + {params[2]:.4f}) + {params[3]:.4f}"
                )
            ...
            fit_label = (
                r"$\alpha = {A:.3f}\cdot\sin ({f:.3f}t+{b:.3f})+{h:.3f}$".format(
                    A=params[0], f=params[1], b=params[2], h=params[3]
                )
            )
            ax.text(
                0.49,
                ...
                verticalalignment="top",
                color=change_luminosity(COLORS[tc], 1.15),
                transform=ax.transAxes,
            )

        out_table.to_csv(
            os.path.join(FIGTAB, "tod_wse_fig10.csv"),
            index_label="hour",
            float_format="%.6f",
        )
        ...
        ax.vlines(
            [time_to_hours(s["sunrise"]), time_to_hours(s["sunset"])],
            linestyle="dashed",
            ymin=ax.get_ylim()[0],
            ymax=ax.get_ylim()[1],
            color="green",
        ...
        if i == 0:
            ...
            ax.set_xticks(
                ticks=major_tick_locations, labels=major_tick_labels, minor=False
            )
            ax.set_xticks(
                np.array(range(24)) + OFFSET * j / 2, range(24), minor=True, size=10
            )
            ax.set_xlabel("Local time (hours)")
    if poster:
        fig.suptitle("Wind Shear Exponent Medians by Time of Day", y=1)
    fig.tight_layout()
    plt.savefig(
        saveto, bbox_inches="tight", edgecolor=fig.get_edgecolor(), transparent=poster
    )
...
def data_gaps(df, cid, summary, size, saveto, poster, details):
    ...
    for i, (b, h) in enumerate(zip(BOOMS_GAPS, HEIGHTS_GAPS), 1):
        ...
        ax.scatter(
            available,
            [(h if GAPS_LINEAR or GAPS_LOGARITHMIC else i) for _ in available],
            s=2,
            c=COLORS["available"],
        )
    ...
    ax.set_xticklabels(
        [
            f"{MONTHS[d.month - 1]}{(' ' + str(d.year)) if (i==0 or month_starts[i].year != month_starts[i-1].year) else ''}"
...
def terrain_breakdown(df, cid, summary, size, saveto, poster, details):
    ...
    for tc in ["open", "complex", "other"]:
        ...
        for i in range(len(MONTHS)):
            plt.text(
                i,
                last.iloc[i] + num.iloc[i] / 2,
                f"{100*perc.iloc[i]:.1f}%",
                ha="center",
                va="center",
            )
        last += num
    ...
    for i in range(12):
        plt.text(
            i,
            last.iloc[i] + top_offset,
            f"{'N=' if i == 0 else ''}{last.iloc[i]}",
            ha="center",
        )
    ax.set_xlabel("Month")
    ...
    if poster:
        fig.suptitle(
            f'Terrain Breakdown Based on Boom {summary["terrain_wind_boom"]} Wind Directions'
        )
    fig.tight_layout()
    plt.savefig(
        saveto, bbox_inches="tight", edgecolor=fig.get_edgecolor(), transparent=poster
...
def windrose_comparison(df, cid, summary, size, saveto, poster, details):
    ...
    speed_bins = [
        ...
        9.0,
        1000,
    ]  # 1000 in place of np.inf (unrealistically high speed value; np.inf gives a RuntimeWarning even though it works)
    cmap = (
        cm.rainbow
    )  # Color map used for bins (note: append suffix _r to use reversed version)
    ...
    for ax in axs:
        ax.set_yticks(
            tick_values,
            ["" if value == 0 else f"{value:.1f}%" for value in tick_values],
        )
    ...
    ] + [f">{speed_bins[-2]:.1f} m/s"]
    patches = [
        plt.Line2D([0], [0], color=colors[i], lw=5) for i in range(len(legend_labels))
    ]
    fig.legend(
        patches,
        legend_labels,
...
def pti_profiles(df, cid, summary, size, saveto, poster, details):
    ...
    for i, tc in enumerate(TERRAINS):
        ...
        for j, sc in enumerate(stabilities):
            ...
            stds = dfs[[f"pti_{b}" for b in BOOMS]].std(axis=0).values
            ax.plot(means, HEIGHTS, color=change_luminosity(COLORS[sc], 0.85), zorder=0)
            ax.scatter(
                means,
                [h - 1.5 + j for h in HEIGHTS],
            ...
            tilabel = f"ti_{tc}_{sc.replace(' ','-')}"
            out_table[tilabel] = means
            out_table[f"rms_ti_{tilabel}"] = stds

        out_table.to_csv(
            os.path.join(FIGTAB, "ti_profiles_fig12.csv"),
            index_label="height",
            float_format="%.6f",
        )
    ...
    if poster:
        fig.suptitle(
            r"Annual Profiles of Turbulence Intensity, by Terrain and Stability"
        )
    fig.tight_layout()
    plt.savefig(
        saveto, bbox_inches="tight", edgecolor=fig.get_edgecolor(), transparent=poster
    ...
    for i, (b, h) in enumerate(zip(BOOMS, HEIGHTS), 1):
        ...
        if DISTS_BY_TERRAIN:
            for j, tc in enumerate(TERRAINS):
                ...
                fit_label = r"$k={k:.2f},~\lambda={lamb:.2f}$".format(
                    k=shape, lamb=scale
                )
                ax.text(
                    0.99,
            ...
            ax.plot(
                UVALS, weib(UVALS), color=change_luminosity(COLORS["default1"], 0.85)
            )
        ...
        if DISTS_BY_TERRAIN:
            for j, tc in enumerate(TERRAINS):
                ...
                if i == 1:
                    ax.legend(loc="lower right")
            ax.vlines(
                x=[df.loc[df["terrain"] == tc, f"pti_{b}"].mean() for tc in TERRAINS],
                ymin=0,
                ymax=ax.get_ylim()[1],
                colors=[change_luminosity(COLORS[tc], 1.5) for tc in TERRAINS],
                alpha=0.7,
            ...
            ax.vlines(
                x=[df.loc[df["terrain"] == tc, f"pti_{b}"].median() for tc in TERRAINS],
                ymin=0,
                ymax=ax.get_ylim()[1],
                colors=[change_luminosity(COLORS[tc], 1.5) for tc in TERRAINS],
                alpha=0.8,
            ...
            ax.vlines(
                ...
                ymin=0,
                ymax=ax.get_ylim()[1],
                colors=[change_luminosity(COLORS["default1"], 1.5)],
                alpha=0.7,
                linestyle="solid",
                linewidth=4,
            )
            ax.vlines(
                x=[df[f"pti_{b}"].median()],
                ymin=0,
                ymax=ax.get_ylim()[1],
                colors=[change_luminosity(COLORS["default1"], 1.5)],
                alpha=0.8,
...
def pti_vs_wse(df, cid, summary, size, saveto, poster, details):
    ...
    ax.scatter(
        ...
        s=2,
        c=COLORS["default2"],
        label="Open Terrain + Neutral Stability",
    )
    ax.legend()
...
def pti_vs_drms(df, cid, summary, size, saveto, poster, details):
    ...
    ax.scatter(
        df[f"pti_{DRMSBOOM}"], df[f"drms_{DRMSBOOM}"], s=0.2, c=COLORS["default1"]
    )
    ...
    ax.set_xlabel(
        r"$\sigma_M/\overline{{M}}$ ({H} meters)".format(
            H=int(HEIGHTS_GAPS[DRMSBOOM - 1])
        )
    )
...
def correlations(df, cid, summary, size, saveto, poster, details):
    ...
    WHICH = [
        ...
        + [f'veer{summary["veer_boom_top"]}-{summary["veer_boom_bottom"]}']
        if col in df.columns
    ]
    corrs = get_correlations(df, which=WHICH)

    im = ax.imshow(corrs, cmap="seismic", vmin=-1.0, vmax=1.0)
...
def determinations(df, cid, summary, size, saveto, poster, details):
    ...
    im = ax.imshow(corrs, cmap="plasma", vmin=0.0, vmax=1.0)
    ax.set_xticks(ticks=range(len(WHICH)), labels=WHICH, rotation=90)
    ax.set_yticks(ticks=range(len(WHICH)), labels=WHICH)
...
def alpha_corrs(df):
    ...
    def get_and_scatter(col, transy, transx="linear", alpha_bounds=(0, 1)):
        ...
        print(
            f"alpha ({transy}) vs {col} ({transx}): {value:.3f} (r^2 = {(value**2):.3f})"
        )
        try:
            plt.scatter(
                stats.TRANSFORMS[transx](dfx[col]), dfx["alpha"], s=0.1, alpha=0.5
            )
            plt.yscale(transy)
            ...
            plt.close()
            plt.scatter(
                stats.TRANSFORMS[transx](dfx[col]),
                stats.TRANSFORMS[transy](dfx["alpha"]),
                s=0.1,
                alpha=0.5,
            )
            plt.ylabel(f"alpha ({transy})")
...
def veer_histograms(df, cid, summary, size, saveto, poster, details):
    ...
    for i, ax in enumerate(fig.axes):
        ...
        ax.text(
            ...
            verticalalignment="top",
            bbox=dict(
                boxstyle="round", facecolor="none", edgecolor=COLORS[sc], linewidth=2
            ),
        )
        if details:
            print(f" {sc}:")
    ...
    if poster:
        fig.suptitle(
            r"Wind Veer Distributions (\+ for veer, - for backing), by Stability"
        )
    fig.tight_layout()
    plt.savefig(
        saveto, bbox_inches="tight", edgecolor=fig.get_edgecolor(), transparent=poster
...
def loglaw_neutral_by_terrain(df, *args, **kwargs):
    COLORS = COLORS_POSTER if kwargs.get("poster") else COLORS_FORMAL
    fig, axs = plt.subplots(ncols=len(TERRAINS), figsize=kwargs["size"])
    dfn = df.loc[df["stability"] == "neutral"]
    for i, tc in enumerate(TERRAINS):
        ax = axs[i]
        if LOGLAW_SEMILOG:
            ax.set_ylim((1, 180))
            ...
            ax.set_ylim((-7.5, 167.5))
        dft = dfn.loc[dfn["terrain"] == tc]
        means = dft[[f"ws_{b}" for b in BOOMS]].mean(axis=0).values
        ustar, z0 = stats.neutral_loglaw_fit(
            HEIGHTS, means, displacement=DISPLACEMENTS[tc]
        )

        err = 0
        for u_actl, u_pred in zip(
            means,
            [
                (
                    (ustar / stats.KAPPA) * np.log((z - DISPLACEMENTS[tc]) / z0)
                    if (z > DISPLACEMENTS[tc])
                    else 0
                )
                for z in HEIGHTS
            ],
        ...
        print(
            f"For terrain class {tc} using displacement d={DISPLACEMENTS[tc]}, got u*={ustar} and z0={z0}, with MSE {err}"
        )
        zvals = ZVALS[ZVALS > DISPLACEMENTS[tc]]
        ax.plot(
            (ustar / stats.KAPPA) * np.log((zvals - DISPLACEMENTS[tc]) / z0),
            zvals,
            color=change_luminosity(COLORS[tc], 0.85),
            zorder=2,
        )
        ax.scatter(
            means,
        ...
        if DISPLACEMENTS[tc] > 0 or not LOGLAW_SEMILOG:
            ax.axhline(
                DISPLACEMENTS[tc],
                color="tab:red",
                linestyle="dashed",
                label=f"d={DISPLACEMENTS[tc]:.2f} m",
            )
        ax.set_title(
            tc.title(), loc="right", x=0.978, y=0.022 if LOGLAW_SEMILOG else 0.14
        )
        ax.set_xlabel("Mean Wind Speed (m/s)")
        ax.xaxis.grid(True, linestyle="--", alpha=0.6)
        if LOGLAW_SEMILOG:
        ...
        ax.legend(loc="upper left")
    plt.tight_layout()
    plt.savefig(kwargs["saveto"])


ALL_PLOTS = {
    "bar_stability": ("Stability Frequency Bar Plot", bar_stability, (4, 3)),
    "annual_profiles": (
        "Annual Wind Profiles with Fits, by Terrain",
        annual_profiles,
        (6.5, 3),
    ),
    "wse_histograms": (
        "Histograms of WSE, by Stability, including Terrain",
        wse_histograms,
        (6.5, 4.5),
    ),
    "veer_histograms": (
        "Histograms of Wind Veer, by Stability",
        veer_histograms,
        (6.5, 4.5),
    ),
    "veer_profiles": ("Wind Direction Profiles, by Terrain", veer_profiles, (6.5, 3)),
    "tod_wse": (
        "Time of Day Plots of WSE, by Terrain, including Stability & Fits",
        tod_wse,
        (6.5, 6),
    ),
    "data_gaps": ("Data Gap Visualization", data_gaps, (8, 4)),
    "terrain_breakdown": (
        "Breakdown of Terrain Characterizations, by Month",
        terrain_breakdown,
        (6.5, 4.5),
    ),
    "windrose_comparison": (
        "KCC/CID Wind Rose Comparison",
        windrose_comparison,
        (6.5, 3),
    ),
    "pti_profiles": ("Annual Pseudo-TI Profiles, by Terrain", pti_profiles, (6.5, 3)),
    "speed_distributions": (
        "Distributions of Wind Speeds, by Height",
        speed_distributions,
        (6.5, 6),
    ),
    "pti_distributions": (
        "Distributions of Pseudo-TI, by Height",
        pti_distributions,
        (6.5, 6),
    ),
    ...
    "pti_vs_drms": (
        f"{int(HEIGHTS_GAPS[DRMSBOOM-1])} meter Pseudo-TI vs Directional RMS",
        pti_vs_drms,
        (6.5, 3),
    ),
    "correlations": ("Correlation Coefficients", correlations, (7, 7)),
    "determinations": ("Determination Coefficients", determinations, (7, 7)),
    "loglaw_neutral_by_terrain": (
        "Log Law Fits for z0 (by Terrain)",
        loglaw_neutral_by_terrain,
        (6.5, 3),
    ),
...
JUSTONE = "veer_profiles"  # "loglaw_neutral_by_terrain"
...
def generate_plots(
    ...
    savedir: str,
    summary: dict,
    which: list = [JUSTONE] if JUSTONE is not None else ALL_PLOTS.keys(),
    poster: bool = False,
    details: bool = False,
    **kwargs,
):
    ...
    if not details:
        print("Details suppressed. Rerun with details = True (-v in kcc.py) to print.")
    ...
    for tag in which:
        ...
        try:
            plotter(
                ...
                summary=summary,
                size=size,
                saveto=f"{fig_savedir}/{tag}.png",
                poster=poster,
                details=details,
            )
            not_generated.remove(tag)
        except Exception as e:
            print(f"FAILED to generate {tag}: {e}")
